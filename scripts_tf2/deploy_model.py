...
def check_ssh_connection() -> bool:
    """KV260にSSH接続できるか事前チェック（タイムアウト5秒）"""
...
def transfer_model(compiled_dir: str, compiled_name: str, model_name: str) -> bool:
    """コンパイル済みモデルをKV260に転送"""
...
使用例:
  ...
  python deploy_model.py --config_name yolov3-512.cfg --weights_name yolov3-512_last.weights --compiled_dir /tmp/out
    ...
    if not transfer_model(compiled_dir, compiled_name, model_name):
        exit(1)
